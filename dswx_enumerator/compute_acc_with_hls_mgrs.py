...
class StatisticWater:
    ...
    def set_classified_data(self,
        ...
        self.metadata = _dswx_sar_util.get_meta_from_tif(image_path)
...
def read_tif_latlon(input_tif_str):
    #  Initialize the Image Size
    ds = gdal.Open(input_tif_str)

    if ds is None:
        raise RuntimeError(f"Cannot open {input_tif_str}")
...
def determine_polygon(input_tif_str, bbox=None):
    ...
    if bbox is not None:
        ...
        poly = read_tif_latlon(input_tif_str)
...
def get_lonlat(xcoord, ycoord, epsg: int):
    """
    Transform a point (x,y) in EPSG:<epsg> to lon/lat in EPSG:4326,
    forcing traditional GIS axis order (X=lon, Y=lat) to avoid GDAL 3+ flips.
    """
    # Fast path: already lon/lat
    if int(epsg) == 4326:
    ...
    lon, lat, *_ = ct.TransformPoint(float(xcoord), float(ycoord))
...
def download_dswx_hls(polys, 
    ...
    # CMR expects West,South,East,North = (min_lon, min_lat, max_lon, max_lat)
    # bounding box is in the following order: min_x, min_y, max_x, max_y
    min_x, min_y, max_x, max_y = polys.bounds

    pad = 0.2

    west  = _clamp(min_x + pad, -180.0, 180.0)
    south = _clamp(min_y + pad,  -90.0,  90.0)
    east  = _clamp(max_x - pad, -180.0, 180.0)
    north = _clamp(max_y - pad,  -90.0,  90.0)

    bounding_box = f"{west},{south},{east},{north}"
    print(f'{bounding_box = }\n')

    temporal = f"{datetime_start_str},{datetime_end_str}"
    ...
    # Diagnostics & defensive handling
    print("[CMR] URL:", response.url)
    print("[CMR] Status:", response.status_code, response.reason)
    hits = response.headers.get('CMR-Hits')
    print("[CMR] CMR-Hits:", hits if hits is not None else "(header missing)")

    if not response.ok:
        print("[CMR] Response (first 800 chars):", response.text[:800])
        return 0, []

    data = response.json()
    entries = data.get("feed", {}).get("entry", [])
    print("[CMR] Returned granules:", len(entries))
...
def run(cfg):
    ...
    for water_s1_path in glob.iglob(f'{input_dir}/OPERA*B01_WTR*'):
        ...
        try:
            ...
            date_obj = datetime.strptime(acquisition_time, "%Y-%m-%dT%H:%M:%SZ")
            #try:
            #    if '.' in acquisition_time:
            #        date_obj = datetime.strptime(acquisition_time, "%Y-%m-%dT%H:%M:%S.%fZ")
            #    else:
            #        date_obj = datetime.strptime(acquisition_time, "%Y-%m-%dT%H:%M:%SZ")
            #except ValueError as ve:
            #    raise ValueError(f"Timestamp format error: {ve}")
